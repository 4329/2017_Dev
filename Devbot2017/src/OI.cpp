...
OI::OI() {
    ...
    outtake.reset(new  JoystickButton(robotOperator.get(), Xbox_Button::Left_Bumper));
    intake.reset(new JoystickButton(robotOperator.get(), Xbox_Button::Right_Bumper));

    //Specify which commands to call when button is pressed, released, etc
    outtake->WhenPressed(new FuelOuttake());
    intake->WhenPressed(new FuelIntake());
