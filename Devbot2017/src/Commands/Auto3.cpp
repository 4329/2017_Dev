...
Auto3::Auto3() {
	...
	float  Step1_MoveTimeout = Preferences::GetInstance()->GetFloat("AutoRightPeg::Step1_MoveTimeout", 2.075);
	float  Step1_Output      = Preferences::GetInstance()->GetFloat("AutoRightPeg::Step1_Output", 0.6);
	float  Step2_Rotate      = Preferences::GetInstance()->GetFloat("AutoRightPeg::Step2_Rotate", -56.0);
	float  Step3_MoveTimeout = Preferences::GetInstance()->GetFloat("AutoRightPeg::Step3_MoveTimeout", 1.25);
	float  Step3_Output      = Preferences::GetInstance()->GetFloat("AutoRightPeg::Step3_Output", 0.6);
	double Step5_Delay       = Preferences::GetInstance()->GetDouble("AutoRightPeg::Step5_Delay", 0.5);
	float  Step6_MoveTimeout = Preferences::GetInstance()->GetFloat("AutoRightPeg::Step6_MoveTimeout", 2.0);
	float  Step6_Output      = Preferences::GetInstance()->GetFloat("AutoRightPeg::Step6_Output", -0.3);
...
//	AddSequential(new MoveStraight_By_Timeout(1.25, 0.6));	//go to right peg
	AddSequential(new MoveStraight_By_Timeout(Step3_MoveTimeout, Step3_Output));	//go to right peg
